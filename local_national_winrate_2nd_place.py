"""
「当地勝率(or 全国勝率)が低い2号艇/3号艇の選手は、2着に来るとは思えない」
という感覚を、実際のデータで検証する。

①②条件を満たし、実際に1号艇が逃げ、2着が2号艇/3号艇だったレースに絞り、
・2号艇/3号艇それぞれの「当地勝率」「全国勝率」の絶対水準と、
  実際にその艇が2着に来たかどうかの関係
・特に「当地勝率が低い(下位グループ)選手」が実際にどれくらいの頻度で
  2着に来ているか
を調べる。
"""
import sqlite3
...
entries_all = pd.read_sql_query(
    "SELECT race_date, jcd, rno, waku, toban, national_win_rate, local_win_rate FROM entries", conn)
...
e2 = entries_all[entries_all["waku"] == 2][["race_date", "jcd", "rno", "national_win_rate", "local_win_rate"]].rename(
    columns={"national_win_rate": "nwr2", "local_win_rate": "lwr2"})
e3 = entries_all[entries_all["waku"] == 3][["race_date", "jcd", "rno", "national_win_rate", "local_win_rate"]].rename(
    columns={"national_win_rate": "nwr3", "local_win_rate": "lwr3"})
...
print("=== 2号艇の当地勝率(絶対水準)を4分位に区切った時、2号艇が実際に2着に来た率 ===")
d = df.dropna(subset=["lwr2"]).copy()
d["q"] = pd.qcut(d["lwr2"], q=4, duplicates="drop")
for b, sub in d.groupby("q", observed=True):
    n = len(sub)
    h = int(sub["target"].sum())
    l, u = wilson_ci(h, n)
    print(f"  2号艇当地勝率{b}: n={n} 2号艇が2着に来た率{h/n*100:.1f}%[{l:.1f}-{u:.1f}]")

print("\n=== 2号艇の全国勝率(絶対水準)を4分位に区切った時、2号艇が実際に2着に来た率 ===")
d = df.dropna(subset=["nwr2"]).copy()
d["q"] = pd.qcut(d["nwr2"], q=4, duplicates="drop")
for b, sub in d.groupby("q", observed=True):
    n = len(sub)
    h = int(sub["target"].sum())
    l, u = wilson_ci(h, n)
    print(f"  2号艇全国勝率{b}: n={n} 2号艇が2着に来た率{h/n*100:.1f}%[{l:.1f}-{u:.1f}]")

print("\n=== 「2号艇の当地勝率が特に低い」ケースだけ抜き出す ===")
for cutoff in [1.0, 2.0, 3.0]:
    sub = df[df["lwr2"] <= cutoff]
    n = len(sub)
    if n < 15:
        print(f"  当地勝率<= {cutoff}: n={n}(少なすぎるためスキップ)")
        continue
    h = int(sub["target"].sum())
    l, u = wilson_ci(h, n)
    print(f"  当地勝率<= {cutoff}: n={n} 2号艇が2着に来た率{h/n*100:.1f}%[{l:.1f}-{u:.1f}]")

print("\n=== 相関係数まとめ(2号艇が2着=1として) ===")
for col, label in [("nwr2", "2号艇全国勝率"), ("lwr2", "2号艇当地勝率"), ("nwr3", "3号艇全国勝率"), ("lwr3", "3号艇当地勝率")]:
    dd = df.dropna(subset=[col])
    r = np.corrcoef(dd[col], dd["target"])[0, 1]
    print(f"  {label}: r={r:+.3f} (n={len(dd)})")

print(f"\n参考: 全体のベースライン(2号艇が2着に来る率) = {df['target'].mean()*100:.1f}% (n={len(df)})")

# 今日の鳴門3Rの2号艇(加藤奈月)の当地勝率3.93は、この分布のどのあたりか
lwr2_all = df["lwr2"].dropna()
pct_rank = (lwr2_all < 3.93).mean() * 100
print(f"\n参考: 当地勝率3.93は、この母集団の2号艇当地勝率の下から{pct_rank:.0f}パーセンタイルに位置する")
