...
「1-6」に100円（1レースあたり合計1,500円）を賭け続けた場合の回収率と、
...
# 買い目ごとの賭け金（円）。1-2〜1-6の全通りをカバーする重み付け。
BET_WEIGHTS = {"1-2": 500, "1-3": 400, "1-4": 300, "1-5": 200, "1-6": 100}
TOTAL_BET_PER_RACE = sum(BET_WEIGHTS.values())  # 1,500円
...
def race_return(row):
    ...
    if weight is None:
        return 0
    # payoutテーブルの値は100円賭けた場合の払戻金。賭け金の倍率をかける。
...
print("買い目: 2連単 1-2(500円) + 1-3(400円) + 1-4(300円) + 1-5(200円) + 1-6(100円)"
      "、1レースあたり計1,500円")
print(f"対象レース数（①②条件に合致し結果判明済み）: {total_races}件")
print(f"的中回数（1-2〜1-6いずれか）: {hit_count}回")
