...
# 現在DBに入っている最古のrace_dateより前の日を、開始境界まで1日ずつ
# 古い方向へ遡って取得する。オッズは対象外(--backfill-oddsで別途扱う)。
#
# 開始境界は既定で直近3年分(公式サイトの現行フォーマットで閲覧できると
# 確認できているのは2016年9月以降だが、1日あたりの実測所要時間が想定より
# 大幅に長く-1レースあたりの応答が数秒単位でかかり、1日(出走表+結果のみ)
# でも約90分かかる-10年分をGitHub Actionsの1ジョブ6時間上限の中で現実的な
# 期間で終えるのは難しいため、対象範囲を直近3年に絞っている)。
#
...
BACKFILL_HISTORY_START = "20230901"
